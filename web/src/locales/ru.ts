...
`,
    ...
    knowledgeConfiguration: {
      ...
      manualSetup: 'Пайплайн',
      builtIn: 'Встроенный',
      titleDescription:
        'Обновите конфигурацию датасета здесь, в частности LLM и промпты.',
      name: 'Название датасета',
      photo: 'Иконка датасета',
      ...
      embeddingModelTip:
        'Модель эмбеддингов по умолчанию для датасета. После появления чанков при смене модели система случайно выбирает несколько чанков для проверки совместимости, перекодирует их новой моделью и вычисляет косинусное сходство. Переключение возможно только если среднее сходство ≥ 0.9. Иначе удалите все чанки в датасете.',
      permissionsTip:
        "Если установлено значение «Команда», все члены команды смогут управлять датасетом.",
      chunkTokenNumberTip:
        'Порог токенов для создания чанка. Сегмент с меньшим числом токенов объединяется со следующими, пока не превысит порог — тогда создаётся чанк. Новый чанк не создаётся без разделителя, даже если порог превышен.',
      chunkMethod: 'Метод чанкинга',
      ...
      methodEmpty:
        'Здесь будет отображено визуальное объяснение категорий датасета',
...
<p>Убедитесь, что вы установили <b>Типы сущностей</b>.</p>`,
      tag: `<p>Датасет с методом чанкинга «Тег» функционирует как набор тегов. Другие датасеты используют его для тегирования чанков, и запросы к ним также тегируются этим набором.</p>
<p>Набор тегов <b>НЕ</b> будет напрямую участвовать в процессе RAG.</p>
<p>Каждый чанк в этом датасете — независимая пара описание-тег.</p>
...
Выше приведено содержимое, которое вам нужно суммировать.`,
      ...
      vietnamese: 'Вьетнамский',
      pageRank: 'Page rank',
      pageRankTip: `Вы можете назначить более высокий балл PageRank определённым датасетам при извлечении. Соответствующий балл добавляется к комбинированным оценкам схожести извлечённых чанков, повышая их рейтинг. Подробности: https://ragflow.io/docs/dev/set_page_rank`,
      tagName: 'Тег',
      frequency: 'Частота',
...
<ul>
  ...
  <li>Авто-ключевые слова зависят от LLM и потребляют много токенов.</li>
      ...
      useGraphRagTip:
        'Строит граф знаний поверх чанков файлов текущего датасета для многошагового Q&A с вложенной логикой. Подробности: https://ragflow.io/docs/dev/construct_knowledge_graph',
    ...
    chat: {
      ...
      chatConfigurationDescription:
        ' Настройте чат-ассистента для ваших выбранных датасетов здесь! 💕',
      ...
      emptyResponseTip: `Установите это как ответ, если по вашему запросу не найдено результатов из датасетов, или оставьте поле пустым, чтобы LLM импровизировала, когда ничего не найдено.`,
      emptyResponseMessage: `Пустой ответ сработает, когда ничего релевантного не извлечено из датасетов. Очистите поле «Пустой ответ», если датасет не выбран.`,
      emptyResponsePlaceholder:
        'Ответ, который вы ищете, не найден в датасете!',
      ...
      knowledgeBasesTip:
        'Выберите датасеты для связи с этим чат-ассистентом. Пустой датасет не появится в списке.',
...
**Основные правила:**
  - Ответ должен опираться **только** на этот датасет: {knowledge}.
  ...
  - **Всегда учитывайте** всю историю диалога.`,
      ...
      freedomTip: `Ярлык для «Температура», «Top P», «Штраф за присутствие» и «Штраф за частоту». «Импровизировать» — более творческие ответы; «Точно» (по умолчанию) — консервативнее; «Баланс» — между ними.`,
      temperature: 'Температура',
      ...
      topPTip:
        'Также «ядерная выборка»: порог для выбора меньшего набора наиболее вероятных слов.',
      ...
      frequencyPenaltyTip:
        'Как штраф за присутствие, снижает частое повторение одних и тех же слов.',
      ...
      fullScreenDescription:
        'Встройте следующий iframe на ваш сайт в нужном месте',
      ...
      tts: 'Текст в речь',
      ttsTip:
        'Выберите модель TTS в настройках перед включением воспроизведения текста в аудио.',
      relatedQuestion: 'Связанный вопрос',
      answerTitle: 'R',
      multiTurn: 'Многошаговая оптимизация',
      multiTurnTip:
        'Оптимизирует запросы с учётом контекста в многораундовом диалоге. Потребляет дополнительные токены LLM.',
      ...
      useKnowledgeGraphTip:
        'Использовать ли граф(ы) знаний в указанных датасетах при извлечении для многошагового Q&A. Включает итеративные поиски по сущностям, связям и отчётам сообществ; увеличивает время извлечения.',
      ...
      reasoningTip: `Включать ли рассуждения при ответе (как в Deepseek-R1 или o1). Позволяет пошагово решать сложные вопросы и использовать внешние знания.`,
      tavilyApiKeyTip:
        'При корректном API ключе Tavily веб-поиск дополняет извлечение из датасетов.',
      ...
      metadata: 'Метаданные',
      metadataTip:
        'Фильтрация по метаданным: теги, категории, права доступа для уточнения извлечения.',
      conditions: 'Условия',
      ...
      tocEnhanceTip: `При парсинге документа было сгенерировано оглавление (см. опцию извлечения оглавления в методе General). Большая модель возвращает пункты оглавления, релевантные запросу, для извлечения чанков и взвешивания при сортировке — по аналогии с поиском в книге.`,
      batchDeleteSessions: 'Массовое удаление',
    ...
    setting: {
      ...
      connectorNameTip: 'Понятное имя для коннектора',
...
Узел агрегации переменных (изначально узел назначения переменных) - это важный узел в рабочем процессе. Он отвечает за интеграцию выходных результатов разных ветвей, гарантируя, что независимо от того, какая ветвь выполнена, ее результат может быть ссылочно доступен через единую переменную. Это чрезвычайно полезно в сценариях с несколькими ветвями, так как он отображает переменные с той же функцией в разных ветвях на одну выходную переменную, избегая избыточных определений в нижестоящих узлах.`,
      ...
      extractorDescription:
        'Использует LLM для извлечения структурированных данных из чанков — сводки, классификации и т.д.',
